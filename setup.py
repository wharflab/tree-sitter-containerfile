...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_containerfile", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_containerfile": ["*.pyi", "py.typed"],
        "tree_sitter_containerfile.queries": ["*.scm"],
    },
    ext_package="tree_sitter_containerfile",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
